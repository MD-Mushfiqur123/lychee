...
## Nvidia
Lychee supports Nvidia GPUs with compute capability 5.0+ and driver version 531 and newer.
...
If you have multiple NVIDIA GPUs in your system and want to limit Lychee to use
...
On linux, after a suspend/resume cycle, sometimes Lychee will fail to discover
...
Lychee supports the following AMD GPUs via the ROCm library:
...
Lychee requires the AMD ROCm v7 driver on Linux. You can install or upgrade
...
Lychee requires an AMD ROCm v7 / HIP7-capable driver stack on Windows.
...
Lychee leverages the AMD ROCm library, which does not support all AMD GPUs. In
...
Reach out on [Discord](https://discord.gg/lychee) or file an
[issue](https://github.com/MD-Mushfiqur123/lychee/issues) for additional help.

### GPU Selection

If you have multiple AMD GPUs in your system and want to limit Lychee to use a
...
Lychee supports GPU acceleration on Apple devices via the Metal API.
...
For AMD GPUs on some Linux distributions, you may need to add the `lychee` user to the `render` group.

The Lychee scheduler leverages available VRAM data reported by the GPU libraries to
...
capability are granted, Lychee will use approximate sizes of the models
to make best effort scheduling decisions.

```bash
sudo setcap cap_perfmon+ep /usr/local/bin/lychee
```

### GPU Selection

To select specific Vulkan GPU(s), you can set the environment variable
`GGML_VK_VISIBLE_DEVICES` to one or more numeric IDs on the Lychee server as
described in the [FAQ](faq#how-do-i-configure-lychee-server). If you
encounter any problems with Vulkan based GPUs, you can disable all Vulkan GPUs
by setting `LYCHEE_VULKAN=0` or `GGML_VK_VISIBLE_DEVICES=-1`.
