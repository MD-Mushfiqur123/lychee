...
<|"|>}<tool_call|>`,
		...
		},
		{
			...
			input: `<|tool_call>call:exec{command:<|"|>fetch "https://lychee.com/library/gemma4" --extract<|"|>}<tool_call|>`,
			expectedToolCalls: []api.ToolCall{
				{
					Function: api.ToolCallFunction{
						Name: "exec",
						Arguments: testArgs(map[string]any{
							"command": `fetch "https://lychee.com/library/gemma4" --extract`,
						}),
					},
				},
			},
...
func TestParseGemma4ToolCall_RepairsIssue15315Examples(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// Source: https://github.com/MD-Mushfiqur123/lychee/issues/15315#issue-4203625511
		...
		},
		{
			...
			tools:   []api.Tool{gemma4TestStringTool("bash", "command")},
			// Source: https://github.com/MD-Mushfiqur123/lychee/issues/15315#issuecomment-4194547092
			want: api.ToolCall{
				Function: api.ToolCallFunction{
					Name: "bash",
					Arguments: testArgs(map[string]any{
